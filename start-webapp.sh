...
if [ ! -f .env ]; then
    ...
    echo "⚠️  Veuillez configurer votre clé API Groq dans le fichier .env"
    exit 1
fi
...
if [ -z "$HAS_GROQ" ]; then
    echo "⚠️  La clé API Groq n'est pas configurée dans le fichier .env"
    echo "📝 Veuillez configurer GROQ_API_KEY"
    exit 1
fi
